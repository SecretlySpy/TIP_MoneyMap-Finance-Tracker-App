import { bytesToHex, getOrCreateDatabaseKey } from "../src/db/keyManager";
...
describe("database key manager", () => {
    ...
    test("creates one 256-bit key and then reuses it", async () => {
        ...
        const randomBytes = jest.fn(async (byteCount) => Uint8Array.from({ length: byteCount }, (_, index) => index));
    ...
    test("fails closed when a stored key is malformed", async () => {
        ...
        await expect(getOrCreateDatabaseKey(keyStore, async () => new Uint8Array(32))).rejects.toThrow("refusing to replace it");
        expect(keyStore.writeCount).toBe(0);
    });
    test("rejects a random provider that returns the wrong byte count", async () => {
        const keyStore = new MemoryKeyStore();
        await expect(getOrCreateDatabaseKey(keyStore, async () => new Uint8Array(31))).rejects.toThrow("invalid key length");
        expect(keyStore.writeCount).toBe(0);
    });
});
