const { injectAndroidOpenSslJniPackaging } = require(
  "../plugins/withAndroidOpenSslJniPackaging",
) as {
  injectAndroidOpenSslJniPackaging: (contents: string) => string;
};
...
describe("Android OpenSSL JNI packaging plugin", () => {
  it("adds the SQLCipher runtime extraction before native-library merging", () => {
    ...
    expect(result.indexOf("prepareMoneyMapOpenSslJni")).toBeLessThan(
      result.indexOf("dependencies {"),
    );
  });
  ...
  it("fails loudly when Expo's Gradle template no longer has the expected anchor", () => {
    expect(() => injectAndroidOpenSslJniPackaging("android {\n}\n")).toThrow(
      "MoneyMap could not locate Android's dependencies block for OpenSSL packaging.",
    );
  });
});
