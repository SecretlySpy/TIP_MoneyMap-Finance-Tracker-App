...
import { smartTips } from "./fixtures";
...
export function SmartTipsScreen({ navigation }) {
    ...
    const weeklySpentMinor = summary.limitMinor > 0
    ...
    const progressPercent = weeklyLimitMinor <= 0 ? 0 : Math.min(100, Math.round((weeklySpentMinor / weeklyLimitMinor) * 100));
    return (<ScreenContainer contentContainerStyle={{ gap: theme.spacing.xl }} testID="smart-tips-screen">
      ...
      {smartTips.map((tip) => (<SectionCard key={tip.id} padding={theme.spacing.lg} style={{ alignItems: "center", flexDirection: "row", gap: theme.spacing.md, minHeight: theme.sizes.tipCard }}>
          <View style={{
                alignItems: "center",
