...
export function ManageCategoriesScreen({ navigation }) {
    ...
    const renderGroup = (title, items) => (<View style={{ gap: theme.spacing.sm }}>
    ...
    return (<ScreenContainer contentContainerStyle={{ gap: theme.spacing.xl }} testID="manage-categories-screen">
      ...
      <TextPromptModal confirmLabel="Add" message="Custom categories stay on this device only." onCancel={() => setPromptType(null)} onConfirm={(value) => void handleAdd(value)} placeholder="Category name" title={promptType === "INCOME" ? "New income category" : "New expense category"} visible={promptType !== null}/>
    </ScreenContainer>);
}
