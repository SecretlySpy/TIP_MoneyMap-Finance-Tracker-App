...
export function RecurringScreen({ navigation }) {
    ...
    const bills = useMemo(() => buildRecurringBills(recurringRules, categoriesById), [recurringRules, categoriesById]);
    ...
    const handleAmountConfirm = async (value) => {
        ...
        const preferred = expenseCategories.find((category) => category.name === "Bills") ?? expenseCategories[0];
    ...
    return (<ScreenContainer contentContainerStyle={{ gap: theme.spacing.xl }} testID="recurring-screen">
      <View style={{ alignItems: "center", flexDirection: "row", gap: theme.spacing.lg }}>
        <Pressable accessibilityLabel="Go back" accessibilityRole="button" hitSlop={theme.spacing.md} onPress={() => navigation.goBack()}>
          ...
          No recurring bills yet. Add one to track upcoming payments offline.
                ...
                Alert.alert(bill.name, "Remove this recurring bill?", [
                    { text: "Cancel", style: "cancel" },
                    {
                        ...
                        style: "destructive",
                        onPress: () => {
                            void deleteRecurringById(Number(bill.id)).catch((error) => {
                                Alert.alert("Delete failed", error instanceof Error ? error.message : "Could not delete bill.");
                            });
                        },
                    },
                ]);
      ...
      <TextPromptModal confirmLabel="Next" message="Name shown on the bills list." onCancel={() => {
            setStep(null);
            setDraftName("");
        }} onConfirm={(value) => {
            setDraftName(value.trim() || "Monthly bill");
            setStep("amount");
        }} placeholder="Internet" title="Bill name" visible={step === "name"}/>
      <TextPromptModal confirmLabel="Save bill" initialValue="1000.00" keyboardType="decimal-pad" message={`Amount for ${draftName || "this bill"}`} onCancel={() => setStep(null)} onConfirm={(value) => void handleAmountConfirm(value)} placeholder="1000.00" title="Bill amount" visible={step === "amount"}/>
    </ScreenContainer>);
}
