...
interface Migration {
  ...
  apply(database: SqlExecutor): Promise<void>;
}
...
async function configureDatabase(database: SqlExecutor): Promise<void> {
...
export async function getSchemaVersion(database: SqlExecutor): Promise<number> {
...
export async function migrateDatabase(database: SqlDatabase): Promise<MigrationResult> {
  ...
  if (previousVersion > LATEST_SCHEMA_VERSION) {
    throw new Error(
      `Database schema version ${previousVersion} is newer than supported version ${LATEST_SCHEMA_VERSION}.`,
    );
  }
