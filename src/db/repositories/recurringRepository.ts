...
function mapRecurringRule(row: SqlRow): RecurringRule {
  return {
...
function validateRecurringRule(rule: NewRecurringRule): void {
...
export class RecurringRepository {
  ...
  public async create(rule: NewRecurringRule): Promise<RecurringRule> {
  ...
  public async getById(id: number): Promise<RecurringRule | null> {
    const row = await findRowById(this.database, "recurring_rules", id);
    return row === null ? null : mapRecurringRule(row);
  }

  public async list(): Promise<RecurringRule[]> {
    return (await listRows(this.database, "recurring_rules")).map(mapRecurringRule);
  }

  public async update(id: number, patch: RecurringRuleUpdate): Promise<RecurringRule | null> {
