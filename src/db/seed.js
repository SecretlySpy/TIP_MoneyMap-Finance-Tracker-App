...
export async function seedInitialData(database) {
    await database.execute(`INSERT INTO accounts (name, type, starting_balance_minor, is_archived)
     SELECT ?, ?, ?, ?
     WHERE NOT EXISTS (
    ...
    for (const category of DEFAULT_STUDENT_CATEGORIES) {
        await database.execute(`INSERT INTO categories (name, icon, color_hex, type, is_custom)
       SELECT ?, ?, ?, ?, ?
       WHERE NOT EXISTS (
